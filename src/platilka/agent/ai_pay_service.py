...
class AIPayService:
    ...
    async def checkout(self, product_url: str, quantity: int, delivery_info: Dict[str, Any], notes: str) -> Dict[
        str, Any]:
        ...
        try:
            ...
            4. Внимательно изучи страницу товара и найди:
               - Точное название товара (обычно в заголовке H1)
               - Актуальную цену за единицу товара в рублях (учитывай скидки и акции)
               - Информацию о наличии товара и максимальном доступном количестве
               - Варианты товара (размер, цвет), если применимо
            ...
            6. Установи количество: {quantity} штук используя наиболее подходящий метод:
               - Для input поля: очисти поле и введи нужное значение
            ...
            5. Заполни контактные данные, ищи поля по следующим критериям:
               ...
               - Ищи поля с атрибутами: name/id содержащими "phone", "tel", "mobile", "telephone"
               ...
               - Комментарий к заказу: оставь пустым или укажи "Заказ через автоматизацию"
            ...
            НЕ НАЖИМАЙ НА КНОПКИ "Оплатить", "Заплатить", "Оплата", "Подтвердить заказ", "Заказать"
            НЕ ПРОИЗВОДИ ФИНАЛЬНУЮ ОПЛАТУ И НЕ ПОДТВЕРЖДАЙ ЗАКАЗ!

            ЭТАП 4: СБОР ИНФОРМАЦИИ И ФОРМАТ ОТВЕТА
